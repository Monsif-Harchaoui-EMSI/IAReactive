/*
  ...
  */
  ...
  class Vehicle {
    ...
    fleeWithTargetRadius(target, radius) {
      let distance;
      // Check if target has a 'pos' property (e.g., Bowser or other Vehicles)
      if (target.pos) {
        distance = this.pos.dist(target.pos);
      } else {
        // If the target is a p5.Vector (e.g., mouse cursor)
        distance = this.pos.dist(target);
      }
      if (distance < radius) {
        const fleeForce = this.flee(target.pos || target); // Handle both types of target
        fleeForce.mult(2);
