class gameVehicle {
  ...
  show() {
    ...
    if (this.img) {
      ...
      imageMode(CENTER);
      image(this.img, 0, 0, this.r * 2, this.r * 2); // Adjust size with `this.r`
    } else {
      // Fallback to default rendering if no image is set
      fill(255);
      stroke(0);
      circle(0, 0, this.r * 2);
    }
...
class Target extends gameVehicle {
  constructor(x, y) {
    super(x, y);
  ...
  show() {
    ...
    if (this.img) {
      ...
      // Fallback if no image is assigned
      fill(255);
      stroke(0);
      circle(0, 0, this.r * 2);
    }
